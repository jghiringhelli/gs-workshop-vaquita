...
export class ContributionService {
  ...
  async recordContribution(tandaId: string, participantId: string, userId: string): Promise<Contribution> {
    ...
    if (!participant || participant.tandaId !== tandaId) {
      throw new Error('Participant not found in this tanda');
    }
    if (participant.userId !== userId) {
      throw new Error('Unauthorized to record contribution for this participant');
    }
   ...
   */
  async getParticipantHistory(participantId: string, userId: string): Promise<Contribution[]> {
    const participant = await this.participantRepository.findById(participantId);
    if (!participant) {
      throw new Error('Participant not found');
    }
    if (participant.userId !== userId) {
      throw new Error('Unauthorized to view this participant history');
    }
